pro cv_image_fill, image,new_image,status,error_message
...
;***********************************************************************
pro cv_centroid_setup_image,cinfo

status = 0
cinfo.centroid.fail = 0


if(cinfo.imagetype eq 0) then begin
    image = (*cinfo.cube.psubdata)
...
if(cinfo.imagetype ge 1) then begin 
    if ptr_valid(cinfo.image2d.psubdata) then begin
        image = (*cinfo.image2d.psubdata) 
...
if ptr_valid(cinfo.centroid.pimage) then ptr_free,cinfo.centroid.pimage
cinfo.centroid.pimage = ptr_new(image)

if ptr_valid(cinfo.centroid.porg_image) then ptr_free,cinfo.centroid.porg_image
cinfo.centroid.porg_image = ptr_new(image)

cinfo.centroid.zoom = zoom
cinfo.centroid.xstart = xstart
cinfo.centroid.ystart = ystart
...
pro cv_centroid,cinfo

if(cinfo.centroid.fail eq 1) then return
xstart = cinfo.centroid.xstart
ystart = cinfo.centroid.ystart
zoom = cinfo.centroid.zoom
image = (*cinfo.centroid.pimage)
...
a[2:5] = a[2:5]/cinfo.centroid.rebin_factor
...
cinfo.centroid.xcenter_plot = a[4] + 0.5 
cinfo.centroid.ycenter_plot = a[5] + 0.5

; for reporting results first pixel is centered at 1.0
cinfo.centroid.xcenter = a[4] + xstart + 1.0
cinfo.centroid.ycenter = a[5] + ystart + 1.0
;print,'in cv_centroid',a[4],a[5],xstart,ystart,cinfo.centroid.xcenter_plot,$
;      cinfo.centroid.ycenter_plot
...
plots,cinfo.centroid.xcenter_plot*zoom,$
      cinfo.centroid.ycenter_plot*zoom,$
      psym=1,/device,color=4,symsize = 1
        

sx = strcompress(string(cinfo.centroid.xcenter, format="(f9.2)"),/remove_all)
sy = strcompress(string(cinfo.centroid.ycenter,format="(f9.2)"),/remove_all)
