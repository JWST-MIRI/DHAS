...
if (widget_info(event.id,/TLB_MOVE_EVENTS) eq 1 ) then begin
    cinfo.centroid.xoffset_surface =  event.x
    cinfo.centroid.yoffset_surface =  event.y
    cinfo.centroid.uoffset_surface =  1
...
;_______________________________________________________________________
    (strmid(event_name,0,5) EQ 'rebin'): begin 
        if(event.index eq 0) then begin
            image = (*cinfo.centroid.porg_image)
        ...
        if(event.index eq 1) then begin 
            ...
            if ptr_valid(cinfo.centroid.pimage) then ptr_free,cinfo.centroid.pimage
            cinfo.centroid.pimage = ptr_new(new_image)
            cinfo.centroid.rebin_factor = 2 * cinfo.centroid.rebin_factor
        endif
...
;_______________________________________________________________________
    (strmid(event_name,0,6) EQ 'rotate'): begin 
        ...
        cinfo.centroid.angle =  angles[event.index]
...
;_______________________________________________________________________
;***********************************************************************
pro cv_centroid_surface_display,cinfo
...
if(cinfo.centroid.uoffset_surface eq 1) then begin
    x_offset = cinfo.centroid.xoffset_surface
    y_offset = cinfo.centroid.yoffset_surface
endif
...
quitbutton = widget_button(quitmenu,value="Quit",event_pro='centroid_surface_quit')
...
cinfo.centroid.angle = 30
cinfo.centroid.do_rebinsmooth = 0
cinfo.centroid.rebin_factor = 1
...
XManager,'surface',SurfacePLot,/No_Block,event_handler = 'surface_event'



widget_control,SurfaceDraw,get_value=tdraw_id
cinfo.centroid.draw_window_id = tdraw_id
...
;_______________________________________________________________________
pro cv_centroid_surface_update,cinfo

xstart = cinfo.centroid.xstart
ystart = cinfo.centroid.ystart

wset,cinfo.centroid.draw_window_id
...
cv_update_spectrum,cinfo ; added this line if the surface plot is up


if(cinfo.imagetype eq 1) then cv_draw_coadd_lines,cinfo
if(cinfo.imagetype eq 0) then  cv_draw_current_wavelength,cinfo
image = 0
