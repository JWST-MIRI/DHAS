    ...
    if printfil eq 0 then begin
	  ...
	  0: Begin  ; write postscript
	     ...
	     Widget_Control, printimageinfo.selectfile, Set_Value = filename
             ...
             if(type eq 0) then cv_update_cube,cinfo,/ps
             if(type eq 1) then cv_update_image2d,cinfo,/ps
...
pro cv_print_image_event, event
  ...
  case event.id of
      ...
      printimageinfo.browseButton: begin
          pout = strcompress(cinfo.control.dirps + '/',/remove_all)
...
otype = 0
path = cinfo.control.dirps
slash = '/'
if(path eq "") then slash = ''
filename = cinfo.control.dirps + slash + cinfo.control.file_cube_base + $
           iname + '.ps'
...
XManager, "cv_printimage", pntrbase, Event_Handler = "cv_print_image_event"
