...
cinfo.cv_wave_play.delay = (100.0 - float(event.value))/50.0 ; max delay of 2 sec
...
if(eventName eq 'WIDGET_BUTTON') then begin 
    case event.id of
        cinfo.cv_wave_play.PrevButton: begin
            ...
            widget_control,cinfo.cv_wave_play.wavelengthID,set_combobox_select=$
...
;_______________________________________________________________________
        cinfo.cv_wave_play.NextButton: begin
            cinfo.view_cube.this_iwavelength = cinfo.view_cube.this_iwavelength+1
...
;_______________________________________________________________________
        cinfo.cv_wave_play.PlayButton: begin
            if(cinfo.cv_wave_play.stopflag eq 0) then begin ; this is executed first  time
                ...
                if(cinfo.view_cube.this_iwavelength gt cinfo.jwst_cube.naxis3) then $
                  cinfo.view_cube.this_iwavelength = cinfo.jwst_cube.naxis3-1
                widget_control,cinfo.cv_wave_play.wavelengthID,set_combobox_select=$
                               cinfo.view_cube.this_iwavelength
            ...
            cinfo.cv_wave_play.stopflag = 0
        end
        cinfo.cv_wave_play.StopButton: begin
            cinfo.cv_wave_play.stopflag= 1
        end
...
if (eventName eq 'WIDGET_TIMER') then begin
    if(cinfo.cv_wave_play.stopflag eq 0) then begin
        wait,cinfo.cv_wave_play.delay

        cinfo.view_cube.this_iwavelength = cinfo.view_cube.this_iwavelength+1
        if(cinfo.view_cube.this_iwavelength ge cinfo.jwst_cube.naxis3) then begin
            cinfo.view_cube.this_iwavelength = cinfo.jwst_cube.naxis3-1
            cinfo.cv_wave_play.stopflag= 1 
        endif else begin 
            widget_control,cinfo.cv_wave_play.wavelengthID,set_combobox_select=$
...
;_______________________________________________________________________
    case event.id of
        cinfo.cv_wave_play.WavelengthID: begin 
...
;_______________________________________________________________________
;***********************************************************************
pro jwst_cv_wavelength_options,cinfo
...
wavelength_label = widget_label(wavelength_box,value='Wavelength',/align_left,$
                                font  = cinfo.font5)
wavelengths = ['              ','              ']
cinfo.cv_wave_play.wavelengthID = widget_combobox(wavelength_box,value = wavelengths,uvalue='wave',$
                               font = cinfo.font5,/dynamic_resize)

wavelength_string = strcompress(string( (*cinfo.jwst_cube.pwavelength)),/remove_all)
nw = n_elements(wavelength_string)
for i = 0, nw -1 do begin
    wavelength_string[i] = strcompress(string(i+1)) + ":   " + wavelength_string[i]

endfor
widget_control,cinfo.cv_wave_play.wavelengthID,set_value = wavelength_string
...
cinfo.cv_wave_play.PrevButton = widget_button(buttonIDs,value='Prev') 
cinfo.cv_wave_play.NextButton = widget_button(buttonIDs,value='Next') 
cinfo.cv_wave_play.PlayButton = widget_button(buttonIDs,value='Play')
cinfo.cv_wave_play.StopButton = widget_button(buttonIDs,value='Stop')
 
sIDs = widget_base(info_box,/row)
cinfo.cv_wave_play.Slider = widget_slider(sIDs,/drag,title='Play Speed',maximum = 100,minimum =10,$
