...
value = (*info.jwst_image_pixel.pixeldata)[i,*]
...
if(info.jwst_image_pixel.file_lc_exist eq 1) then begin
    lc = (*info.jwst_image_pixel.lc_pixeldata)
endif

if(info.jwst_image_pixel.file_mdc_exist eq 1) then begin
    mdc = (*info.jwst_image_pixel.mdc_pixeldata)
 endif

if(info.jwst_image_pixel.file_reset_exist eq 1) then begin
    reset = (*info.jwst_image_pixel.reset_pixeldata)
 endif

if(info.jwst_image_pixel.file_rscd_exist eq 1) then begin
    rscd = (*info.jwst_image_pixel.rscd_pixeldata)
 endif


if(info.jwst_image_pixel.file_lastframe_exist eq 1) then begin
    lastframe = (*info.jwst_image_pixel.lastframe_pixeldata)
endif


nend = info.jwst_image_pixel.nframes
...
for j = 0,nend-1 do begin
    ...
    if(info.jwst_image_pixel.hex eq 1) then  begin
    ...
    if(info.jwst_image_pixel.nints eq info.jwst_image_pixel.integrationNO ) then begin
        sramp = 'NA'
        sref = 'NA'
    endif    
...
;_______________________________________________________________________
    ...
    (strmid(event_name,0,7) EQ 'display') : begin
        if(event.index eq 0) then info.jwst_image_pixel.hex = 0
        if(event.index eq 1) then info.jwst_image_pixel.hex = 1
...
pix_statFormat = ["I3", "I4", "I4", "F16.5","F16.5","I8","F16.2"]
...
slope_exist = info.jwst_image_pixel.slope_exist
if(info.jwst_image_pixel.integrationNO+1 gt info.jwst_image_pixel.nslopes) then slope_exist = 0


if(slope_exist) then begin 
    signal  = info.jwst_image_pixel.slope
    zeropt =  info.jwst_image_pixel.zeropt
...
if(info.jwst_image_pixel.file_refcorrection_exist eq 1) then $
  value_refcorrected = (*info.jwst_image_pixel.refcorrected_pixeldata)[i,*]

value_lc  =0
if(info.jwst_image_pixel.file_lc_exist eq 1) then begin
  value_lc = (*info.jwst_image_pixel.lc_pixeldata)[i,*]
endif

value_mdc  =0
if(info.jwst_image_pixel.file_mdc_exist eq 1) then begin
  value_mdc = (*info.jwst_image_pixel.mdc_pixeldata)[i,*]
endif

value_reset  =0
if(info.jwst_image_pixel.file_reset_exist eq 1) then begin
  value_reset = (*info.jwst_image_pixel.reset_pixeldata)[i,*]
endif

value_rscd  =0
if(info.jwst_image_pixel.file_rscd_exist eq 1) then begin
  value_rscd = (*info.jwst_image_pixel.rscd_pixeldata)[i,*]
endif

value_lastframe  =0
if(info.jwst_image_pixel.file_lastframe_exist eq 1) then begin
  value_lastframe = (*info.jwst_image_pixel.lastframe_pixeldata)[i]
endif
...
if(info.jwst_image_pixel.coadd eq 0)then pix_statLabelID = widget_label(pixelinfo,$
...
for j = 0,nend-1 do begin
    ...
    if(info.jwst_image_pixel.file_refcorrection_exist eq 0) then  begin
    ...
    if(info.jwst_image_pixel.file_lc_exist eq 0) then begin
        slc_value = 'NA'
    endif else begin 
    ...
    if(info.jwst_image_pixel.file_mdc_exist eq 0) then begin
        smdc_value = 'NA'
    endif else begin 
    ...
    if(info.jwst_image_pixel.file_reset_exist eq 0) then begin
        sreset_value = 'NA'
    endif else begin 
        sreset_value = strtrim(string(value_reset[j],format="("+pix_statFormat[4]+")"),2)
     endelse

    if(info.jwst_image_pixel.file_rscd_exist eq 0) then begin
        srscd_value = 'NA'
    endif else begin 
    ...
    if(info.jwst_image_pixel.file_lastframe_exist eq 0 or j+1 ne nend) then begin
        slastframe_value = 'NA'
    endif else begin 
    ...
    if(j+1 lt info.jwst_image_pixel.start_fit or j+1 gt info.jwst_image_pixel.end_fit) then begin
        srefcorrected_value = 'NA'
...
if(info.jwst_image_pixel.file_refcorrection_exist ne 0) then $
pix_statLabelID = widget_list(pix2,$
                              value=refcorrected_value,/align_left,scr_ysize=200,$
                              uvalue ='refc')


if(info.jwst_image_pixel.file_mdc_exist eq 1) then $
