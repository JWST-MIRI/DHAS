...
pro Channel_cslice_print, event
	  ...
	  0: Begin  ; write postscript
              ...
              for i = 0,4 do begin
                  ...
                  if(printsliceinfo.type eq 1)then $
                    mql_update_Channel_colslice,graphno,printsliceinfo.cinfo,/ps
                  if(printsliceinfo.type eq 2)then $
                    mql_update_SlopeChannel_colslice,graphno,printsliceinfo.cinfo,/ps
              endfor
	     ...
	     set_plot, 'ps', /copy
             ...
             for i = 0,4 do begin
                 ...
                 graphno = i
                 if(printsliceinfo.type eq 1)then $
                   mql_update_Channel_colslice,graphno,printsliceinfo.cinfo,/eps

                 if(printsliceinfo.type eq 2)then $
                   mql_update_SlopeChannel_colslice,graphno,printsliceinfo.cinfo,/eps
...
pro print_Channel_cslice,cinfo
...
outname = cinfo.outname
  ...
  if(XRegistered("mql_printCcslice")) then return
...
; widget window parameters
  ...
  if(type eq 1) then grouplead =  info.CSliceChannelRawQuickLook
  if(type eq 2) then grouplead =  info.CSliceSlopeChannelQuickLook
  ...
  selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 120, /Edit, $
		    Event_Pro = 'Channel_cslice_print')
  ...
  XManager, "mql_printCcslice", pntrbase, Event_Handler = "print_event"
          
end

;***********************************************************************

pro Channel_cslice_print_data, event
  ...
  if fcount gt 0 then begin
    ...
    if printfil eq 0 then begin
        ...
        for i = 0,4 do begin
            ...
            graphno = i
            if(printsliceinfo.type eq 1)then $
              mql_update_Channel_colslice,graphno,printsliceinfo.cinfo,/ascii,unit=lun

            if(printsliceinfo.type eq 2)then $
              mql_update_SlopeChannel_colslice,graphno,printsliceinfo.cinfo,/ascii,unit=lun
            close,lun
            free_lun,lun
        endfor
...
pro print_Channel_cslice_data,cinfo
...
outname = cinfo.outname
  ...
  ; Pop up a small widget so the user can type in a file name.
  ; Wait for the user to type a carriage-return.
  if(XRegistered("mql_printCcslice_data")) then return
...
; widget window parameters
  ...
  label      = Widget_Label (pntr1base, Value='Output file name:') 
  selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 120, /Edit, $
		    Event_Pro = 'Channel_cslice_print_data')
  pntr2base  = Widget_Base  (pntrbase, /Row)
  ...
  XManager, "mql_printCcslice_data", pntrbase, Event_Handler = "print_data_event"
