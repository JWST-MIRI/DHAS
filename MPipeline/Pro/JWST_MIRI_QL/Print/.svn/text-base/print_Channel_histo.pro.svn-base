	  ...
	  0: Begin  ; write postscript
              ...
              for i = 0,4 do begin
                  ...
                  if(printhistinfo.type eq 1)then $
                    mql_update_Channel_histo,graphno,printhistinfo.hinfo,/ps
                  if(printhistinfo.type eq 2)then $
                    mql_update_SlopeChannel_histo,graphno,printhistinfo.hinfo,/ps
              endfor
	     ...
	     set_plot, 'ps', /copy
             ...
             for i = 0,4 do begin
                 ...
                 graphno = i
                 if(printhistinfo.type eq 1)then $
                   mql_update_Channel_histo,graphno,printhistinfo.hinfo,/eps

                 if(printhistinfo.type eq 2)then $
                   mql_update_SlopeChannel_histo,graphno,printhistinfo.hinfo,/eps
...
outname = hinfo.outname
  ...
  if(XRegistered("mql_printChist")) then return
...
; widget window parameters
  ...
  if(type eq 1) then grouplead =  info.HistoChannelRawQuickLook
  ...
  XManager, "mql_printChist", pntrbase, Event_Handler = "print_event"
          
end

;***********************************************************************

pro Channel_hist_print_data, event
  ...
  if fcount gt 0 then begin
    ...
    if printfil eq 0 then begin
        ...
        for i = 0,4 do begin
            ...
            graphno = i
            if(printhistinfo.type eq 1)then $
              mql_update_Channel_histo,graphno,printhistinfo.hinfo,/ascii,unit=lun

            if(printhistinfo.type eq 2)then $
              mql_update_SlopeChannel_histo,graphno,printhistinfo.hinfo,/ascii,unit=lun
            close,lun
            free_lun,lun
        endfor
...
pro print_Channel_histo_data,hinfo
...
; widget window parameters
  ...
  label      = Widget_Label (pntr1base, Value='Output file name:') 
  selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 120, /Edit, $
		    Event_Pro = 'Channel_hist_print_data')
  pntr2base  = Widget_Base  (pntrbase, /Row)
  ...
  XManager, "mql_printChist_data", pntrbase, Event_Handler = "print_data_event"
