...
pro compare_colslice_print, event
    ...
    if printfil eq 0 then begin
	  ...
	  0: Begin  ; write postscript
              ...
              for i = 0,2 do begin
                  ...
                  newname = newname +'_'+ strcompress(string(i+1),/remove_all)
                  ...
                  if(printhistinfo.type eq 1)then $
                    mql_update_compare_colslice,graphno,printhistinfo.hinfo,/ps
                  if(printhistinfo.type eq 2)then $
                    msql_update_compare_colslice,graphno,printhistinfo.hinfo,/ps
              endfor
	     ...
	     set_plot, 'ps', /copy
             ...
             for i = 0,2 do begin
                 ...
                 graphno = i
                 if(printhistinfo.type eq 1)then $
                   mql_update_compare_colslice,graphno,printhistinfo.hinfo,/eps

                 if(printhistinfo.type eq 2)then $
                   msql_update_compare_colslice,graphno,printhistinfo.hinfo,/eps
...
;_______________________________________________________________________
; Main Print graphs
pro print_compare_colslice,hinfo
...
outname = hinfo.outname
  ...
  if(XRegistered("mql_printColSliceCompare")) then return
...
; widget window parameters
  ...
  if(type eq 1) then grouplead =  info.ColSliceCompareQuickLook
  if(type eq 2) then grouplead =  info.ColSliceSCompareQuickLook
  ...
  selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 120, /Edit, $
		    Event_Pro = 'compare_colslice_print')
  ...
  XManager, "mql_printColSliceCompare", pntrbase, Event_Handler = "print_event"
...
;_______________________________________________________________________
; Event Handler for printing Data
;_______________________________________________________________________
pro compare_colslice_print_data, event
  ...
  if fcount gt 0 then begin
    ...
    if printfil eq 0 then begin
        ...
        for i = 0,2 do begin
            ...
            graphno = i
            if(printhistinfo.type eq 1)then $
              mql_update_compare_colslice,graphno,printhistinfo.hinfo,/ascii,unit=lun

            if(printhistinfo.type eq 2)then $
              msql_update_compare_colslice,graphno,printhistinfo.hinfo,/ascii,unit=lun
            close,lun
            free_lun,lun
        endfor
...
pro print_compare_colslice_data,hinfo
...
outname = hinfo.outname
  ...
  ; Pop up a small widget so the user can type in a file name.
  ; Wait for the user to type a carriage-return.
  if(XRegistered("mql_PrintColSliceCompare_data")) then return
...
; widget window parameters
  ...
  title      = 'MIRI Quicklook Print Column Slice Data to File'
  pntrbase   = Widget_Base  (Title = title, /Column, Group_Leader=grouplead, $
                           xsize = xwidget_size,$
                           ysize = ywidget_size,/scroll,$
                           x_scroll_size= xsize_scroll,$
  ...
  label      = Widget_Label (pntr1base, Value='Output file name:') 
  selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 120, /Edit, $
		    Event_Pro = 'compare_colslice_print_data')
  pntr2base  = Widget_Base  (pntrbase, /Row)
  ...
  XManager, "mql_printColSliceCompare_data", pntrbase, Event_Handler = "print_data_event"
