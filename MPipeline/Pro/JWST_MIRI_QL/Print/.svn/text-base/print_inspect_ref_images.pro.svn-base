...
pro inspect_ref_images_print, event
...
;_______________________________________________________________________
pro print_inspect_ref_images_event, event
...
;_______________________________________________________________________
pro print_inspect_ref_images,info 


  ; Pop up a small widget so the user can type in a file name.
  ; Wait for the user to type a carriage-return.
if(XRegistered("mirql_printimage")) then return

iframe = info.inspect_ref.FrameNO+1
jintegration = info.inspect_ref.integrationNO+1
...
outname = info.output.inspect_ref  + '_' + ij
mtitle = 'MIRI Quicklook Print Reference Output  Image'
...
pntrbase   = Widget_Base  (Title = mtitle, /Column, $
                           Group_Leader=info.InspectRefImage, $
;			      /Modal)
                           xsize = xwidget_size,$
...
selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 120, /Edit, $
                           Event_Pro = 'inspect_ref_images_print')
...
XManager, "mirql_printimage", pntrbase, Event_Handler = "print_inspect_ref_images_event"
