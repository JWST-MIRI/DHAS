...
pro inspect_slope_images_print, event
...
;_______________________________________________________________________
pro print_inspect_slope_images,info 
...
jintegration = info.inspect_slope.integrationNO+1
ij = strcompress('int'+string(jintegration),/remove_all)


mtitle = ' ' 

if(info.slope.plane[0] eq 0) then xt = 'Slope'
if(info.slope.plane[0]  eq 1) then xt = 'Uncertainty'
if(info.slope.plane[0] eq 2) then xt = 'Data_Quality_Flag'
if(info.slope.plane[0] eq 3) then xt = 'Zero_Pt'
if(info.slope.plane[0] eq 4) then xt = 'Good_Frames'
if(info.slope.plane[0] eq 5) then xt = '1st_Saturated_Frame'
if(info.slope.plane[0] eq 6) then xt = 'Max_2pt_Diff' 
if(info.slope.plane[0] eq 7) then xt = 'Frame_num_max_2pt_diff
if(info.slope.plane[0] eq 8) then xt = 'Stdev_2pt_diff'
if(info.slope.plane[0] eq 9) then xt = 'Slope_2pt_diff'

outname = info.output.inspect_slope + xt + '_' + ij
...
pntrbase   = Widget_Base  (Title = mtitle, /Column, $
                           Group_Leader=info.InspectSlope, $
;			      /Modal)
                           xsize = xwidget_size,$
...
selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 100, /Edit, $
                           Event_Pro = 'inspect_slope_images_print')
...
XManager, "miql_printimage", pntrbase, Event_Handler = "print_event"
