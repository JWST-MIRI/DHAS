    ...
    if printfil eq 0 then begin
	  ...
	  0: Begin  ; write postscript
	     ...
	     Widget_Control, printchannelinfo.selectfile, Set_Value = filename
             ...
             for i = 0, 4 do begin
                 mean = info.ChannelR[i].sd_mean
                 min = info.ChannelR[i].sd_min
                 max = info.ChannelR[i].sd_max
                 ...
                 xyouts,xloc[i],-15,'Channel '+strcompress(string(fix(i+1)),/remove_all),charsize=0.75
	     ...
	     Widget_Control, printchannelinfo.selectfile, Set_Value = filename
             ...
             wset,info.Channel.draw_window_id[0]
             im1 = tvrd(true=1)
             wset,info.Channel.draw_window_id[1]
             im2 = tvrd(true=1)
             wset,info.Channel.draw_window_id[2]
             im3 = tvrd(true=1)
             wset,info.Channel.draw_window_id[3]
             im4 = tvrd(true=1)
             wset,info.Channel.draw_window_id[4]
             im5 = tvrd(true=1)
...
; widget window parameters
  ...
  ij = 'int' + string(fix(info.ChannelR[0].jintegration+1))+'_frame' + string(fix(info.ChannelR[0].iramp+1))
  ij = strcompress(ij,/remove_all)

  outname = info.output.channel + '_' + ij

  filename = info.control.dirps + slash + info.control.filebase + $
             outname  + '.ps'

  title      = 'MIRI Quicklook Print 5 Channel Images'
  pntrbase   = Widget_Base  (Title = title, /Column, Group_Leader=info.RawChannelQuickLook, $
;			      /Modal)
                           xsize = xwidget_size,$
  ...
  XManager, "mql_printchannel", pntrbase, Event_Handler = "print_channel_event"
