...
pro compare_rowslice_print, event
	  ...
	  0: Begin  ; write postscript
              ...
              for i = 0,2 do begin
                  ...
                  if(printhistinfo.type eq 1)then $
                    mql_update_compare_rowslice,graphno,printhistinfo.hinfo,/ps
                  if(printhistinfo.type eq 2)then $
                    msql_update_compare_rowslice,graphno,printhistinfo.hinfo,/ps
              endfor
	     ...
	     set_plot, 'ps', /copy
             ...
             for i = 0,2 do begin
                 ...
                 graphno = i
                 if(printhistinfo.type eq 1)then $
                   mql_update_compare_rowslice,graphno,printhistinfo.hinfo,/eps

                 if(printhistinfo.type eq 2)then $
                   msql_update_compare_rowslice,graphno,printhistinfo.hinfo,/eps
...
;_______________________________________________________________________
; Main Print graphs
pro print_compare_rowslice,hinfo
...
outname = hinfo.outname
  ...
  if(XRegistered("mql_printRowsliceCompare")) then return
...
; widget window parameters
  ...
  if(type eq 1) then grouplead =  info.RowsliceCompareQuickLook
  if(type eq 2) then grouplead =  info.RowsliceSCompareQuickLook
  ...
  selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 120, /Edit, $
		    Event_Pro = 'compare_rowslice_print')
  ...
  XManager, "mql_printRowsliceCompare", pntrbase, Event_Handler = "print_event"
...
;_______________________________________________________________________
pro compare_rowslice_print_data, event
  ...
  if fcount gt 0 then begin
    ...
    if printfil eq 0 then begin
        ...
        for i = 0,2 do begin
            ...
            graphno = i
            if(printhistinfo.type eq 1)then $
              mql_update_compare_rowslice,graphno,printhistinfo.hinfo,/ascii,unit=lun

            if(printhistinfo.type eq 2)then $
              msql_update_compare_rowslice,graphno,printhistinfo.hinfo,/ascii,unit=lun
            close,lun
            free_lun,lun
        endfor
...
pro print_compare_rowslice_data,hinfo
...
outname = hinfo.outname
  ...
  ; Pop up a small widget so the user can type in a file name.
  ; Wait for the user to type a carriage-return.
  if(XRegistered("mql_PrintRowsliceCompare_data")) then return
...
; widget window parameters
  ...
  label      = Widget_Label (pntr1base, Value='Output file name:') 
  selectfile = Widget_Text  (pntr1base, Value = filename, XSize = 120, /Edit, $
		    Event_Pro = 'compare_rowslice_print_data')
  pntr2base  = Widget_Base  (pntrbase, /Row)
  ...
  XManager, "mql_printRowsliceCompare_data", pntrbase, Event_Handler = "print_data_event"
