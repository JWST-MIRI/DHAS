...
pro jwst_mcql_draw_zoom_box,info
;_______________________________________________________________________
ij = info.jwst_cal.zoom_window-1
wset,info.jwst_cal.draw_window_id[ij]

xstart_plot = info.jwst_cal.x_zoom_start/info.jwst_cal.binfactor
xrange  = info.jwst_cal.x_zoom_end - info.jwst_cal.x_zoom_start + 1
xend_plot = xstart_plot + (xrange/info.jwst_cal.binfactor)

ystart_plot = info.jwst_cal.y_zoom_start/info.jwst_cal.binfactor
yrange  = info.jwst_cal.y_zoom_end - info.jwst_cal.y_zoom_start + 1
yend_plot = ystart_plot + (yrange/info.jwst_cal.binfactor) 
...
;_______________________________________________________________________
pro jwst_mcql_update_zoom_image,info,ps = ps,eps = eps
;_______________________________________________________________________
...
x = info.jwst_cal.x_zoom
y = info.jwst_cal.y_zoom

; initialize the x and y pos to be the center of the image

info.jwst_cal.x_zoom_pos = info.jwst_cal.x_zoom
info.jwst_cal.y_zoom_pos = info.jwst_cal.y_zoom
widget_control,info.jwst_cal.graph_label[2], set_value=szoom
...
info.jwst_cal.zoom_xplot_size = xsize
info.jwst_cal.zoom_yplot_size = ysize
...
info.jwst_cal.x_zoom_start = xstart
info.jwst_cal.y_zoom_start = ystart
info.jwst_cal.y_zoom_end = yend
info.jwst_cal.x_zoom_end = xend
...
stat_data = 0
;_______________________________________________________________________
if ptr_valid (info.jwst_cal.pzoomdata) then ptr_free,info.jwst_cal.pzoomdata
info.jwst_cal.pzoomdata = ptr_new(sub_image)

info.jwst_cal.zoom_stat[0] = image_mean
info.jwst_cal.zoom_stat[1] = stdev_pixel
info.jwst_cal.zoom_stat[2] = image_min
info.jwst_cal.zoom_stat[3] = image_max
info.jwst_cal.zoom_stat[4] = image_median
info.jwst_cal.zoom_stat[5] = stdev_mean

widget_control,info.jwst_cal.graphID[2],draw_xsize = xrange*zoom,draw_ysize=yrange*zoom 

if(hcopy eq 0) then wset,info.jwst_cal.pixmapID[2]
disp_image = congrid(sub_image, xsize*zoom,ysize*zoom)
disp_image = bytscl(disp_image,min=info.jwst_cal.graph_range[2,0], $
                    max=info.jwst_cal.graph_range[2,1],$
...
if(hcopy eq 0) then begin 
    wset,info.jwst_cal.draw_window_id[2]
    device,copy=[0,0,$
                 xsize*zoom,$
                 ysize*zoom, $
                 0,0,info.jwst_cal.pixmapID[2]]
endif else begin
    stitle = "Zoom " + szoom
...
scale_min = info.jwst_cal.graph_range[2,0]
scale_max = info.jwst_cal.graph_range[2,1]

widget_control,info.jwst_cal.rlabelID[2,0],set_value=scale_min
widget_control,info.jwst_cal.rlabelID[2,1],set_value=scale_max
...
widget_control,info.jwst_cal.slabelID[2],set_value=('Mean: ' +smean) 
widget_control,info.jwst_cal.mlabelID[2],set_value=(' Min: ' +smin + '  Max: ' +smax) 

; replot the pixel location
xvalue = (x - xstart + ixstart)*info.jwst_cal.scale_zoom
yvalue = (y - ystart + iystart)*info.jwst_cal.scale_zoom

info.jwst_cal.ixstart_zoom = ixstart
info.jwst_cal.xstart_zoom = xstart

info.jwst_cal.iystart_zoom = iystart
info.jwst_cal.ystart_zoom = ystart

pixelsize  = 1.0 * info.jwst_cal.scale_zoom

info.jwst_cal.x_zoom_plotpt = xvalue
info.jwst_cal.y_zoom_plotpt = yvalue
xcenter = fix(xvalue/info.jwst_cal.scale_zoom)* info.jwst_cal.scale_zoom + pixelsize/2.0
ycenter = fix(yvalue/info.jwst_cal.scale_zoom)* info.jwst_cal.scale_zoom + pixelsize/2.0
...
;***********************************************************************
;_______________________________________________________________________
pro jwst_mcql_update_zoom_pixel_location,xvalue,yvalue,info
;_______________________________________________________________________
...
info.jwst_cal.x_pos = x/info.jwst_cal.binfactor
info.jwst_cal.y_pos = y/info.jwst_cal.binfactor
...
info.jwst_cal.x_zoom_pos  = info.jwst_cal.x_pos * info.jwst_cal.binfactor
info.jwst_cal.y_zoom_pos  = info.jwst_cal.y_pos * info.jwst_cal.binfactor


halfpixel = 0.5*  info.jwst_cal.scale_zoom
