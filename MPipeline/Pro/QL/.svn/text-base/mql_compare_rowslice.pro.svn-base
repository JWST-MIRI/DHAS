...
pro mql_compare_rowslice_event,event
...
if (widget_info(event.id,/TLB_SIZE_EVENTS) eq 1 ) then begin
    minfo.compare_rowslice.xwindowsize = event.x
    minfo.compare_rowslice.ywindowsize = event.y
    minfo.compare_rowslice.uwindowsize = 1
    widget_control,event.top,set_uvalue = hinfo
    widget_control,hinfo.info.Quicklook,set_uvalue = minfo
    mql_compare_rowslice,minfo
    return
endif
...
;check if the <> buttons were used
	...
	for i = 0,2 do begin
        	...
        	mql_update_compare_rowslice,i,hinfo
	endfor
...
; channels scale individually
        ...
        if(hinfo.scaleimage-1 eq 3 or k eq 2) then begin
            ...
            mql_update_compare_rowslice,k,hinfo
        endif else begin 
...
;_______________________________________________________________________
 (strmid(event_name,0,2) EQ 'hd') : begin
        ...
        if(hinfo.scaleimage-1 eq 3 or graphno eq 2) then begin
            ...
            mql_update_compare_rowslice,graphno,hinfo
...
;***********************************************************************
pro mql_update_compare_rowslice,imageno,hinfo,ps=ps,eps=eps,ascii=ascii,unit=iunit
...
scol = ' Row range: ' + strcompress(string(fix(x1)),/remove_all) + ' - ' + $
...
plot,xvalues,rowdata,xtitle = "COL  #", ytitle=yt,$
...
;***********************************************************************
pro mql_compare_rowslice,info

window,1,/pixmap
wdelete,1
if( XRegistered ('Crowslice')) then begin
    widget_control,info.RowsliceCompareQuickLook,/destroy
endif



s = size( (*info.compare_image[0].pdata))
maxsize = s[2]
...
stitle = "MIRI Quick Look- Row Slice of Comparison Science Frame Images" + info.version
...
if(info.compare_rowslice.uwindowsize eq 1) then begin ; user has set window size 
    xsize_scroll = info.compare_rowslice.xwindowsize
    ysize_scroll = info.compare_rowslice.ywindowsize
endif
...
quitbutton = widget_button(quitmenu,value="Quit",event_pro='mql_compare_rowslice_quit')
...
XManager,'Cprowslice',RowsliceQuickLook,/No_Block,event_handler='mql_compare_rowslice_event'
...
info.RowsliceCompareQuickLook = RowsliceQuickLook
Widget_Control,info.RowsliceCompareQuickLook,Set_UValue=hinfo
Widget_Control,info.QuickLook,Set_UValue=info
for i = 0,2 do begin
    imageno = i
    mql_update_Compare_rowslice,imageno,hinfo
endfor
Widget_Control,info.RowsliceCompareQuickLook,Set_UValue=hinfo
