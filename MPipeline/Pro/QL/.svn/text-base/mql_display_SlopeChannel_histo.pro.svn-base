...
pro mql_SlopeChannel_histo_quit,event
...
pro mql_SlopeChannel_histo_event,event
...
if (widget_info(event.id,/TLB_SIZE_EVENTS) eq 1 ) then begin
    minfo.channel_Shisto.xwindowsize = event.x
    minfo.channel_Shisto.ywindowsize = event.y
    minfo.channel_Shisto.uwindowsize = 1
...
case 1 of
    (strmid(event_name,0,3) EQ 'Bin') : begin
        ...
        mql_update_SlopeChannel_histo,graphno,hinfo

    end


    (strmid(event_name,0,6) EQ 'printP') : begin
        print_Channel_histo,hinfo
        
    end    

    (strmid(event_name,0,6) EQ 'printD') : begin
        print_Channel_histo_data,hinfo
...
(strmid(event_name,0,5) EQ 'hist_') : begin
        ...
        if(strmid(event_name,7,1) EQ '1') then p = 0 else p = 1
        print,'p',p
;_______________________________________________________________________
; channels scale individually
        ...
        if(hinfo.scalechannel-1 eq 5) then begin
            ...
            mql_update_SlopeChannel_histo,k,hinfo
        endif
 if(hinfo.scalechannel-1 ne 5) then begin
            ...
            for i = 0,4 do begin
                ...
                widget_control,event.top,set_uvalue = hinfo
                widget_control,hinfo.info.Quicklook,set_uvalue = minfo
                mql_update_SlopeChannel_histo,i,hinfo
            endfor
        endif
    end
...
; only call if scaling to a particular channel
        if(hinfo.scalechannel-1 ne 5) then $
          mql_update_Slopechannel_histo,hinfo.scalechannel-1,hinfo
...
;***********************************************************************
pro mql_update_SlopeChannel_histo,graphno,hinfo,ps=ps,eps=eps,ascii=ascii,unit=iunit
; info.ChannelS[graphno].psubdata is filled in in mql_grab_SlopeChannel_images.pro 
...
frame_image = (*info.ChannelS[graphno].psubdata_noref) ; we do not what the reference pixels in histogram
xstart_new = info.slopechannel.xstart_zoom+1
xend_new = info.slopechannel.xend_zoom+1
...
if( finite(info.ChannelS[graphno].sd_median) ) then begin  
    md = info.ChannelS[graphno].sd_median
endif else begin
    md = 1
endelse
...
sd = info.ChannelS[index].sd_stdev
md_use = info.ChannelS[index].sd_median
...
if(hinfo.default_scale_histo[index] eq 0) then begin
    ...
    xhistomin = md_use- sd*2
    xhistomax = md_use +sd*2  
...
;min_value = min(h)
;max_value = max(h)
;hinfo.xrange_default[graphno,0] = xmin
;hinfo.xrange_default[graphno,1] = xmax
;hinfo.yrange_default[graphno,0] = min_value
;hinfo.yrange_default[graphno,1] = max_value
;hinfo.histo_xrange[graphno,*] = hinfo.histo_xrange[index,*]
;hinfo.histo_yrange[graphno,*] = hinfo.histo_yrange[index,*]
...
;    hinfo.histo_xrange[graphno,0] = hinfo.xrange_default[index,0]
;    hinfo.histo_xrange[graphno,1] = hinfo.xrange_default[index,1]
;endif
                                                                                
;if(hinfo.default_scale_histo[index,1] eq 1) then begin
;    hinfo.histo_yrange[graphno,0] = hinfo.yrange_default[index,0]
;    hinfo.histo_yrange[graphno,1] = hinfo.yrange_default[index,1]
;endif
...
if(hcopy eq 1) then begin
    stitle = ' Histogram for Reduced Channel ' + strcompress(string(graphno+1),/remove_all)
endif
    stitle = ' Histogram for Reduced Channel ' + strcompress(string(graphno+1),/remove_all)


if(graphno eq 4 and info.data.sloperef_exist eq 0) then begin
    print,' Slope image for channel 5 does not exist'

    ypt = (y2 + y1)/2.0
    xyouts,x1,ypt,'No Slope Image for Channel 5'
    xyouts,x1,y1,'Run miri_sloper with +Or option'
    return
endif
plot,xnew,h,psym=10,xtitle= xt,ytitle=yt,$
        yrange = [y1,y2],xrange=[x1,x2],$
        ystyle=1,xstyle = 1,xticks = 3,title = stitle,xtickformat = '(f8.3)'
...
;***********************************************************************
pro mql_display_SlopeChannel_histo,info

window,1,/pixmap
wdelete,1
if( XRegistered ('SChisto')) then begin
    widget_control,info.HistoSlopeChannelQuickLook,/destroy
endif
...
stitle = "MIRI Quick Look- 5 Channel Histogram of Reduced Image" + info.version
svalue = " 5 Channel Histogram of Reduced Image Valuess:  "
...
if(info.channel_Shisto.uwindowsize eq 1) then begin ; user has set window size 
    xsize_scroll = info.channel_Shisto.xwindowsize
    ysize_scroll = info.channel_Shisto.ywindowsize
endif
...
; add quit button
quitbutton = widget_button(quitmenu,value="Quit",event_pro='mql_SlopeChannel_histo_quit')
...
histo_xrange        = fltarr(5,2) ; x  plot range
histo_yrange        = fltarr(5,2) ; x  plot range
xrange_default        = lonarr(5,2) ; x  plot range
yrange_default        = lonarr(5,2) ; x  plot range
...
;_______________________________________________________________________
;*****
;graph 1 Histrogram of Channel 1 
;*****
titleID = widget_label(graphID11, value = " Histogram: Channel 1 ",$
...
;_______________________________________________________________________
;*****
;graph 2 Histrogram of Channel 2 
;*****
titleID = widget_label(graphID12, value = " Histogram: Channel 2 ",$
                       /align_center,font=info.font3)
...
rawmedian = info.ChannelS[1].median
...
;_______________________________________________________________________
;*****
;graph 2 Histrogram of Channel 3 
;*****
...
rawmedian = info.ChannelS[2].median
...
;_______________________________________________________________________
;*****
;graph 3 Histrogram of Channel 4 
;*****
titleID = widget_label(graphID21, value = " Histogram:  Channel 4 ",$
                       /align_center,font=info.font3)
...
rawmedian = info.ChannelS[3].median
...
;_______________________________________________________________________
;*****
;graph 5 Histrogram of Channel 5 - Reference Image
;*****
titleID = widget_label(graphID22, value = " Histogram: Channel 5 ",$
                       /align_center,font=info.font3)
...
rawmedian = info.ChannelS[4].median
...
XManager,'SChisto',HistoQuickLook,/No_Block,event_handler='mql_SlopeChannel_histo_event'
...
outname = info.output.Channelhistoslope  + ij + '_'
...
info.HistoSlopeChannelQuickLook = HistoQuickLook
Widget_Control,info.HistoSlopeChannelQuickLook,Set_UValue=hinfo
Widget_Control,info.QuickLook,Set_UValue=info
for i = 0,4 do begin
    graphno = i
    mql_update_SlopeChannel_histo,graphno,hinfo
