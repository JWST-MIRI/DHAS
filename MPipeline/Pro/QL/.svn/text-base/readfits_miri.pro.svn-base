...
; NAME:
;       READFITS_MIRI
; Copy of astro library routine readfits.pro, adapted to work with lae
; PURPOSE:
...
;-
function READFITS_MIRI, filename, header, heap, CHECKSUM=checksum, $ 
...
;       pcount = sxpar(header,'PCOUNT')
       bitpix = long64(sxpar(header,'BITPIX'))
       naxis  = long64(sxpar(header,'NAXIS'))
       gcount = long64(sxpar(header,'GCOUNT')) > 1l
       pcount = long64(sxpar(header,'PCOUNT'))
                
       if naxis GT 0l then begin 
            dims = long64(sxpar( header,'NAXIS*') )          ;Read dimensions
            ndata = long64(dims[0])
            if naxis GT 1l then for i = 2l, naxis do ndata = ndata*dims[i-1l]
                        
                endif else ndata = 0l
                
                nbytes = (abs(bitpix) / 8l) * gcount * (pcount + ndata)
...
;  file
      ...
      if ext LT exten_no then begin
                nrec = long64((nbytes + 2879l) / 2880l)
...
;  help,ndata
;  help,bitpix
  nbytes = long64(abs(bitpix)/8l) * gcount * (pcount + ndata)
;  help,nbytes
