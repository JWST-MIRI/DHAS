...
pro reframp_print, event

 Widget_Control, event.top, Get_UValue=printreframp
 Widget_Control, printreframp.pinfo.info.Quicklook, Get_UValue=info

      ; Get the file name the user typed in.
  Widget_Control, printreframp.selectfile, Get_Value = filename
  printreframp.filename = filename
  Widget_Control, event.top, Set_UValue=printreframp
  ...
  if fcount gt 0 then begin
    ...
    if printfil eq 0 then begin
        ...
        Case (printreframp.otype) of
            
	  0: Begin  ; write postscript
              set_plot, 'ps', /copy
	     filename = disk + path + name + '.ps'
	     Widget_Control, printreframp.selectfile, Set_Value = filename
             device, /landscape, file=filename, /color, encapsulated=0



              rpixel_update_plot,pinfo,/ps
             device,/close
             set_plot, 'x'
           end
	     ...
	     Widget_Control, printreframp.selectfile, Set_Value = filename
              ...
              wset,printreframp.pinfo.draw_window_id
...
Widget_Control, printreframp.pinfo.info.Quicklook, Get_UValue=info
  ...
  case event.id of
      printreframp.otypeButtons: begin
          otype = event.value
          printreframp.otype = otype

          fileold = printreframp.filename
          filenew = fileold
          ...
          printreframp.filename = filenew
          Widget_Control, printreframp.selectfile, set_value=filenew
      end
      
      printreframp.browseButton: begin
          ...
          Widget_Control, printreframp.selectfile, set_value=realpath
          printreframp.filename = realpath
      end

      printreframp.cancelButton: begin
          ptype = 'x'
          set_plot, ptype
...
; widget window parameters
  ...
  title      = 'MIRI Print Reference Output Ramp'
  ...
  printreframp = {selectfile    :     selectfile,   $
  ...
  Widget_Control, pntrbase, set_uvalue = printreframp
  Widget_Control, pntrbase, /Realize

  XManager, "printreframp", pntrbase, Event_Handler = "print_reframp_event"
