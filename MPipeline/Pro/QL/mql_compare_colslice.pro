...
pro mql_compare_colslice_event,event
...
if (widget_info(event.id,/TLB_SIZE_EVENTS) eq 1 ) then begin
    minfo.compare_colslice.xwindowsize = event.x
    minfo.compare_colslice.ywindowsize = event.y
    minfo.compare_colslice.uwindowsize = 1
    widget_control,event.top,set_uvalue = hinfo
    widget_control,hinfo.info.Quicklook,set_uvalue = minfo
    mql_compare_colslice,minfo
    return
endif
...
;check if the <> buttons were used
	...
	for i = 0,2 do begin
        	...
        	mql_update_compare_colslice,i,hinfo
	endfor
...
; channels scale individually
        ...
        if(hinfo.scaleimage-1 eq 3 or k eq 2) then begin
            ...
            mql_update_compare_colslice,k,hinfo
        endif else begin 
...
;_______________________________________________________________________
 (strmid(event_name,0,2) EQ 'hd') : begin
        ...
        if(hinfo.scaleimage-1 eq 3 or graphno eq 2) then begin
            ...
            mql_update_compare_colslice,graphno,hinfo
...
;***********************************************************************
pro mql_update_compare_colslice,imageno,hinfo,ps=ps,eps=eps,ascii=ascii,unit=iunit
...
;***********************************************************************
pro mql_compare_colslice,info

window,1,/pixmap
wdelete,1
if( XRegistered ('Ccolslice')) then begin
    widget_control,info.ColsliceCompareQuickLook,/destroy
endif
...
stitle = "MIRI Quick Look- Column Slice of Comparison Science Frame Images" + info.version
...
if(info.compare_colslice.uwindowsize eq 1) then begin ; user has set window size 
    xsize_scroll = info.compare_colslice.xwindowsize
    ysize_scroll = info.compare_colslice.ywindowsize
endif
...
quitbutton = widget_button(quitmenu,value="Quit",event_pro='mql_compare_colslice_quit')
...
;colnum = info.data.image_xsize/2	
...
XManager,'Cpcolslice',ColsliceQuickLook,/No_Block,event_handler='mql_compare_colslice_event'
...
outname = 'colslice_compare_' + ij + '_' 
...
info.ColsliceCompareQuickLook = ColsliceQuickLook
Widget_Control,info.ColsliceCompareQuickLook,Set_UValue=hinfo
Widget_Control,info.QuickLook,Set_UValue=info
for i = 0,2 do begin
    imageno = i
    mql_update_Compare_colslice,imageno,hinfo
endfor
Widget_Control,info.ColsliceCompareQuickLook,Set_UValue=hinfo
