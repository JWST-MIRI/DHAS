...
pro mql_Channel_colslice_quit,event
...
pro mql_Channel_colslice_event,event
...
if (widget_info(event.id,/TLB_SIZE_EVENTS) eq 1 ) then begin
    minfo.channel_colslice.xwindowsize = event.x
    minfo.channel_colslice.ywindowsize = event.y
    minfo.channel_colslice.uwindowsize = 1
...
;_______________________________________________________________________
    (strmid(event_name,0,1) EQ 'm') : begin
        ...
        mql_update_Channel_colslice,graphno,cinfo
        Widget_Control,event.top,Set_UValue=cinfo
...
;_______________________________________________________________________
    (strmid(event_name,0,3) EQ 'slc') : begin
        ...
        if(cinfo.showline eq 1  ) then begin ; true - turn to false
            ...
            cinfo.showline = 1
            plot_col1 = cinfo.colnum_start -minfo.channel.xstart_zoom  
            plot_col2 = cinfo.colnum_end - minfo.channel.xstart_zoom
            plot_col1 = plot_col1* minfo.channel.zoom
            plot_col2 = plot_col2* minfo.channel.zoom
        endelse
            mql_draw_Channel_colslice,cinfo.showline,plot_col1,plot_col2,minfo
        Widget_Control,event.top,Set_UValue=cinfo
    end
...
;_______________________________________________________________________
    (strmid(event_name,0,3) EQ 'col') : begin
        ...
        minvalue = minfo.channel.xstart_zoom+1
        maxvalue = minfo.channel.xend_zoom+1
;________________
...
;__________________________________________________________________________
        ...
        for i = 0,4 do begin
            mql_update_Channel_colslice,i,cinfo
        endfor
...
;***********************************************************************
pro mql_draw_Channel_colslice,showline,value1,value2,info
...
for i = 0, 4 do begin 
    ...
    if(showline eq 1) then begin
        ...
        color6
        n_reads = info.channel.yplotsize
...
;***********************************************************************
pro mql_update_Channel_colslice,graphno,cinfo,ps=ps,eps=eps,ascii=ascii,unit=iunit
;***********************************************************************
hcopy = 0
...
x1 = cinfo.colnum_start -info.channel.xstart_zoom -1 
x2 = cinfo.colnum_end - info.channel.xstart_zoom -1


coldataT = (*info.ChannelR[graphno].psubdata)[x1:x2,*]
...
xvalues = indgen(n_reads) + 1 + cinfo.info.channel.ystart_zoom

minvalue = cinfo.info.channel.ystart_zoom
maxvalue = cinfo.info.channel.yend_zoom
pad = 0.002
xpad = pad*minvalue
...
plot,xvalues,coldata,xtitle = "ROW  #", ytitle='DN',$
...
;***********************************************************************
pro mql_display_Channel_colslice,info
...
stitle = "MIRI Quick Look- 5 Channel Column Slice Plots" + info.version
svalue = " 5 Channel Column Slice Plots:  "
...
if(info.channel_colslice.uwindowsize eq 1) then begin ; user has set window size 
    xsize_scroll = info.channel_colslice.xwindowsize
    ysize_scroll = info.channel_colslice.ywindowsize
endif
...
; add quit button
quitbutton = widget_button(quitmenu,value="Quit",event_pro='mql_Channel_colslice_quit')
...
titleID = widget_label(graphID11, value = " Column Slice: Channel 1 ",$
...
titleID = widget_label(graphID12, value = " Column Slice: Channel 2 ",$
                       /align_center,font=info.font3)
...
;*****
titleID = widget_label(graphID13, value = " Column Slice: Channel 3 ",$
                       /align_center,font=info.font3)
...
;graph3 Column Slice  of Channel 3 
;*****
titleID = widget_label(graphID21, value = " Column Slice: Channel 4 ",$
                       /align_center,font=info.font3)
...
;*****
titleID = widget_label(graphID22, value = " Column Slice: Channel 5 ",$
                       /align_center,font=info.font3)
...
XManager,'Ccslice',CSliceChannelQuickLook,/No_Block,$
        event_handler='mql_Channel_colslice_event'
...
outname = info.output.Channelcslice  + ij + '_'
type = 1
...
info.CSliceChannelRawQuickLook = CSliceChannelQuickLook
Widget_Control,info.CSliceChannelRawQuickLook,Set_UValue=cinfo
Widget_Control,info.QuickLook,Set_UValue=info
for i = 0,4 do begin
    graphno = i
    mql_update_Channel_colslice,graphno,cinfo
