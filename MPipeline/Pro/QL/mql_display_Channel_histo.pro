...
pro mql_Channel_histo_quit,event
...
pro mql_Channel_histo_event,event
...
if (widget_info(event.id,/TLB_SIZE_EVENTS) eq 1 ) then begin
    minfo.channel_histo.xwindowsize = event.x
    minfo.channel_histo.ywindowsize = event.y
    minfo.channel_histo.uwindowsize = 1
...
case 1 of
    (strmid(event_name,0,3) EQ 'Bin') : begin
        ...
        mql_update_Channel_histo,graphno,hinfo
...
; channels scale individually
        ...
        if(hinfo.scalechannel-1 eq 5) then begin
            ...
            mql_update_Channel_histo,k,hinfo
        endif
	    ...
	    ; check min & max values;
            ...
            for i = 0,4 do begin
                ...
                widget_control,event.top,set_uvalue = hinfo
                widget_control,hinfo.info.Quicklook,set_uvalue = minfo
                mql_update_Channel_histo,i,hinfo
            endfor
        endif
...
;_______________________________________________________________________
    ...
    (strmid(event_name,0,6) EQ 'ascale') : begin
        ...
        if(hinfo.scalechannel-1 ne 5) then  mql_update_channel_histo,hinfo.scalechannel-1,hinfo
...
;***********************************************************************
pro mql_update_Channel_histo,graphno,hinfo,ps=ps,eps=eps,ascii=ascii,unit=iunit
...
frame_image = (*info.ChannelR[graphno].psubdata_noref) ; we do not what the reference pixels in histogram


xstart_new = info.channel.xstart_zoom+1
xend_new = info.channel.xend_zoom+1
...
md = info.ChannelR[graphno].sd_median
...
sd = info.ChannelR[index].sd_stdev
md_use = info.ChannelR[index].sd_median
...
stitle = ' ' 
stitle = ' Histogram for Channel ' + strcompress(string(graphno+1),/remove_all)
...
;***********************************************************************
pro mql_display_Channel_histo,info

window,1,/pixmap
wdelete,1
if( XRegistered ('Chisto')) then begin
    widget_control,info.HistoChannelRawQuickLook,/destroy
endif
...
stitle = "MIRI Quick Look- 5 Channel Histogram of Science Frame Image" + info.version
svalue = " 5 Channel Histogram of Scinece Frame Values:  "
...
if(info.channel_histo.uwindowsize eq 1) then begin ; user has set window size 
    xsize_scroll = info.channel_histo.xwindowsize
    ysize_scroll = info.channel_histo.ywindowsize
endif
...
; add quit button
quitbutton = widget_button(quitmenu,value="Quit",event_pro='mql_Channel_histo_quit')
...
rawmedian = info.ChannelR[0].median
...
rawmedian = info.ChannelR[1].median
...
rawmedian = info.ChannelR[2].median
...
rawmedian = info.ChannelR[3].median
...
rawmedian = info.ChannelR[4].median
...
XManager,'Chisto',HistoQuickLook,/No_Block,event_handler='mql_Channel_histo_event'
...
outname = info.output.Channelhistoraw  + ij + '_'
...
info.HistoChannelRawQuickLook = HistoQuickLook
Widget_Control,info.HistoChannelRawQuickLook,Set_UValue=hinfo
Widget_Control,info.QuickLook,Set_UValue=info
for i = 0,4 do begin
    graphno = i
    mql_update_Channel_histo,graphno,hinfo
