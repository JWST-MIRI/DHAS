...
pro mql_Channel_rowslice_quit,event
...
pro mql_Channel_rowslice_event,event
...
if (widget_info(event.id,/TLB_SIZE_EVENTS) eq 1 ) then begin
    minfo.channel_rowslice.xwindowsize = event.x
    minfo.channel_rowslice.ywindowsize = event.y
    minfo.channel_rowslice.uwindowsize = 1
...
;_______________________________________________________________________
    (strmid(event_name,0,1) EQ 'm') : begin
        ...
        mql_update_Channel_rowslice,graphno,cinfo
        Widget_Control,event.top,Set_UValue=cinfo
...
;_______________________________________________________________________
    (strmid(event_name,0,3) EQ 'slc') : begin
        ...
        if(cinfo.showline eq 1  ) then begin ; true - turn to false
            ...
            cinfo.showline = 1
            plot_row1 = cinfo.rownum_start -minfo.channel.ystart_zoom  
            plot_row2 = cinfo.rownum_end - minfo.channel.ystart_zoom
         ;   print,' row 1 and 2', plot_row1,plot_row2
            plot_row1 = plot_row1* minfo.channel.zoom
            plot_row2 = plot_row2* minfo.channel.zoom
          ;  print,' row 1 and 2', plot_row1,plot_row2
        endelse
            mql_draw_Channel_rowslice,cinfo.showline,plot_row1,plot_row2,minfo
        Widget_Control,event.top,Set_UValue=cinfo
    end
...
;_______________________________________________________________________
    (strmid(event_name,0,3) EQ 'row') : begin
        minvalue = minfo.channel.ystart_zoom + 1
        maxvalue = minfo.channel.yend_zoom + 1
...
;__________________________________________________________________________
        ...
        for i = 0,4 do begin
            mql_update_Channel_rowslice,i,cinfo
        endfor
...
;***********************************************************************
pro mql_draw_Channel_rowslice,showline,value1,value2,info
...
;    print,value1,value2
    ...
    if(showline eq 1) then begin
        n_reads = info.channel.xplotsize
...
;***********************************************************************
pro mql_update_Channel_rowslice,graphno,cinfo,ps=ps,eps=eps,ascii=ascii,unit=iunit
;***********************************************************************
...
y1 = (cinfo.rownum_start) -info.channel.ystart_zoom  -1
y2 = (cinfo.rownum_end) - info.channel.ystart_zoom-1



rowdataT = (*info.ChannelR[graphno].psubdata)[*,y1:y2]

s = size(*info.ChannelR[graphno].psubdata)
...
xvalues = indgen(n_reads) + 1 + cinfo.info.channel.xstart_zoom

minvalue = cinfo.info.channel.xstart_zoom
maxvalue = cinfo.info.channel.xend_zoom
pad = 0.002
xpad = pad*minvalue
...
rowdata_noref = (*info.ChannelR[graphno].psubdata_noref)[0,*,y1:y2]
...
if(hcopy eq 1) then begin
    stitle = ' Row Slice for Channel ' + strcompress(string(graphno+1),/remove_all)
endif
    stitle = ' Row Slice for Channel ' + strcompress(string(graphno+1),/remove_all)


plot,xvalues,rowdata,xtitle = "COLUMN  #", ytitle='DN',$
  xrange=[xx1,xx2],yrange=[yy1,yy2],xstyle =1,ystyle=1,title = stitle,subtitle = sstitle,ytickformat='(f7.0)'
...
;***********************************************************************
pro mql_display_Channel_rowslice,info

window,1,/pixmap
wdelete,1
if( XRegistered ('Crslice')) then begin
    widget_control,info.RSliceChannelRawQuickLook,/destroy
endif
...
ftitle = "Integration #: " + strtrim(string(fix(jintegration+1)),2) + $
            "   Frame #: " + strtrim(string(fix(iramp+1)),2)     

sxmin = strcompress(string(info.channel.xstart_zoom+1),/remove_all)
sxmax = strcompress(string(info.channel.xend_zoom+1),/remove_all)
symin = strcompress(string(info.channel.ystart_zoom+1),/remove_all)
symax = strcompress(string(info.channel.yend_zoom+1),/remove_all)
sregion = "Available Region to Plot: xrange: " + sxmin + " - " + sxmax + " yrange: " + $
	symin + "  - " + symax 

stitle = "MIRI Quick Look- 5 Channel Row Slice Plots" + info.version
svalue = " 5 Channel Row Slice Plots:  "
...
if(info.channel_rowslice.uwindowsize eq 1) then begin ; user has set window size 
    xsize_scroll = info.channel_rowslice.xwindowsize
    ysize_scroll = info.channel_rowslice.ywindowsize
endif
...
CSliceChannelQuickLook = widget_base(title=stitle ,$
                                     col = 1,mbar = menuBar,$
                                     group_leader = info.RawChannelQuickLook,$
                                     xsize = xwidget_size,$
...
; add quit button
quitbutton = widget_button(quitmenu,value="Quit",event_pro='mql_Channel_rowslice_quit')
...
rownum = info.Channel.yposfull
;print,'x and y ',info.Channel.xposfull, info.Channel.yposfull
rownum_start = rownum
rownum_end = rownum
rownum = 1
...
;*****
;graph 1 Row Slice for  Channel 1 
;*****


titleID = widget_label(graphID11, value = " Row Slice: Channel 1 ",$
...
;graph 2 Row Slice for  Channel 1 
;*****


titleID = widget_label(graphID12, value = " Row Slice: Channel 2 ",$
                       /align_center,font=info.font3)
...
;_______________________________________________________________________
;*****
;graph3 Row Slice  of Channel 3 
;*****
titleID = widget_label(graphID13, value = " Row Slice: Channel 3 ",$
                       /align_center,font=info.font3)
...
;graph3 Row Slice  of Channel 3 
;*****
titleID = widget_label(graphID21, value = " Row Slice: Channel 4 ",$
                       /align_center,font=info.font3)
...
;_______________________________________________________________________
;*****
;graph 5 Row Slice  of Channel 5 
;*****
titleID = widget_label(graphID22, value = " Row Slice: Channel 5 ",$
                       /align_center,font=info.font3)
...
XManager,'Crslice',CSliceChannelQuickLook,/No_Block,$
        event_handler='mql_Channel_rowslice_event'
...
ij = 'int' + string(fix(jintegration+1)) + '_frame' + string(fix(iramp+1))  
ij = strcompress(ij,/remove_all)

outname = info.output.Channelrslice  + ij + '_'
type = 1
...
info.RSliceChannelRawQuickLook = CSliceChannelQuickLook
Widget_Control,info.RSliceChannelRawQuickLook,Set_UValue=cinfo
Widget_Control,info.QuickLook,Set_UValue=info
for i = 0,4 do begin
    graphno = i
    mql_update_Channel_rowslice,graphno,cinfo
