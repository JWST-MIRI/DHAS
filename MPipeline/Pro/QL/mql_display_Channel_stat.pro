...
;***********************************************************************
pro mql_Channel_stat_quit,event
...
pro mql_display_Channel_stat,info
...
stitle = "Statistics on Image by Channel"
...
for i = 0,4 do begin
    image_xsize = info.ChannelR[i].xsize  
    image_ysize = info.ChannelR[i].ysize  
    channel_image = (*info.ChannelR[i].pdata)     
    channel_badpixel = (*info.ChannelR[i].pbadpixel)
    ...
    if (info.channel.apply_bad eq 0) then begin
        nbadpixels = 0
        ngoodpixels = n_elements(channel_image)
    endif
...
for i = 0,4 do begin
    sd_mean[i] = info.ChannelR[i].sd_mean
    sd_st_pixel[i] = info.ChannelR[i].sd_stdev
    sd_var[i] = sd_st_pixel[i] * sd_st_pixel[i]
    sd_min[i] = info.ChannelR[i].sd_min
    sd_max[i] = info.ChannelR[i].sd_max
    sd_median[i] = info.ChannelR[i].sd_median
    sd_st_mean[i] = info.ChannelR[i].sd_stdev_mean
    sd_skew[i] = info.ChannelR[i].sd_skew
...
quitbutton = widget_button(quitmenu,value="Quit",event_pro='mql_Channel_stat_quit')

;_______________________________________________________________________
sxmin = strcompress(string(info.ChannelR[0].ximage_range[0]),/remove_all)
sxmax = strcompress(string(info.ChannelR[0].ximage_range[1]),/remove_all)
symin = strcompress(string(info.ChannelR[0].yimage_range[0]),/remove_all)
symax = strcompress(string(info.ChannelR[0].yimage_range[1]),/remove_all)
...
sbad =     ' # Bad Pixels   '+strtrim( string(nbad[0],format="(i8)"),2)
...
sbad =     ' # Bad Pixels   '+strtrim( string(nbad[1],format="(i8)"),2)
...
sbad =     ' # Bad Pixels   '+strtrim( string(nbad[2],format="(i8)"),2)
...
sbad =     ' # Bad Pixels   '+strtrim( string(nbad[3],format="(i8)"),2)
...
if(info.channel.zoom ne 1) then begin
    xmin_image = info.ChannelR[0].sd_ximage_range[0]
    xmax_image = info.ChannelR[0].sd_ximage_range[1]
    ...
    symin = strcompress(string(info.ChannelR[0].sd_yimage_range[0]),/remove_all)
    symax = strcompress(string(info.ChannelR[0].sd_yimage_range[1]),/remove_all)
...
info.StatChannelInfo = StatChannelInfo

stat = {info                  : info}	


Widget_Control,info.StatChannelInfo,Set_UValue=stat
widget_control,info.StatChannelInfo,/realize

XManager,'mchstat',StatChannelInfo,/No_Block
