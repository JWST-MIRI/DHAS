...
if(type eq 0) then begin ; Raw type data
    ...
    info.histoR.xsize = info.data.image_xsize 
    info.histoR.ysize = info.data.image_ysize

    info.histoR.ximage_range[0]  = 1
    info.histoR.ximage_range[1]  = info.data.image_xsize
    info.histoR.yimage_range[0]  = 1
    info.histoR.yimage_range[1]  = info.data.image_ysize

    info.histoR.iramp = info.image.rampNO
    info.histoR.jintegration = info.image.integrationNO
    ...
    if(info.data.subarray eq 0) then begin
        frame_image_noref = frame_image[4:info.data.image_xsize-5,*]
        frame_image = 0
        frame_image = frame_image_noref
        info.histoR.ximage_range[0]  = 5
        info.histoR.ximage_range[1]  = 1028
        frame_image_noref = 0
    endif
...
if(type eq 1) then begin ; zoom image
    ...
    xsize = info.image.zoom_xplot_size
    ysize = info.image.zoom_yplot_size



    info.histoZ.xsize = xsize
    info.histoZ.ysize = ysize

    info.histoZ.iramp = info.image.rampNO
    info.histoZ.jintegration = info.image.integrationNO
    

    xsize = info.image.x_zoom_end_noref - info.image.x_zoom_start_noref + 1
    ysize = info.image.y_zoom_end - info.image.y_zoom_start +1
    ...
    info.histoZ.ximage_range[0]  = x1+1
    info.histoZ.ximage_range[1]  = x2+1
    info.histoZ.yimage_range[0]  = info.image.y_zoom_start+1
    info.histoZ.yimage_range[1]  = info.image.y_zoom_end+1
...
if(type eq 2) then begin ; Slope  data
    ...
    info.histoS.xsize = info.data.slope_xsize 
    info.histoS.ysize = info.data.slope_ysize

    info.histoS.ximage_range[0]  = 1
    info.histoS.ximage_range[1]  = info.data.slope_xsize
    info.histoS.yimage_range[0]  = 1
    info.histoS.yimage_range[1]  = info.data.slope_ysize

    info.histoS.iramp = info.image.rampNO
    info.histoS.jintegration = info.image.integrationNO

    frame_image = fltarr(info.data.slope_xsize,info.data.slope_ysize)
    frame_image[*,*] = (*info.data.preduced)[*,*,0]

    if(info.data.subarray eq 0) then begin
        frame_image_noref = frame_image[4:info.data.slope_xsize-5,*]
        frame_image = 0
        frame_image = frame_image_noref
        info.histoS.ximage_range[0]  = 5
        info.histoS.ximage_range[1]  = 1028
        frame_image_noref = 0
    endif
...
if(type eq 0) then begin ; Science data
    if ptr_valid (info.histoR.pdata) then ptr_free,info.histoR.pdata
    info.histoR.pdata = ptr_new(frame_image)
endif
if(type eq 1) then begin ; Zoom  data
    if ptr_valid (info.histoZ.pdata) then ptr_free,info.histoZ.pdata
    info.histoZ.pdata = ptr_new(frame_image)
endif
if(type eq 2) then begin ; Slope  data
    if ptr_valid (info.histoS.pdata) then ptr_free,info.histoS.pdata
    info.histoS.pdata = ptr_new(frame_image)
endif
frame_image = 0
end
;***********************************************************************
;_______________________________________________________________________
pro mql_update_histo,hinfo,ps=ps,eps=eps,ascii=ascii,unit=iunit
;_______________________________________________________________________
hcopy = 0
...
if(type eq 0) then begin
    frame_image = (*hinfo.info.histoR.pdata)
endif
if(type eq 1) then begin
    frame_image = (*hinfo.info.histoZ.pdata)
endif
if(type eq 2) then begin
    frame_image = (*hinfo.info.histoS.pdata)
...
if(hcopy eq 1) then begin 
    stitle = hinfo.subt
    if(type lt 2) then sstitle = hinfo.info.control.filename_raw
    if(type eq 2 ) then sstitle = hinfo.info.control.filename_slope
endif
...
if(type eq 1 and hinfo.info.image.zoom_window eq 3) then plot_slope = 1
...
if (widget_info(event.id,/TLB_SIZE_EVENTS) eq 1 ) then begin
    ...
    if(type eq 0) then begin
        info.histoR.xwindowsize = event.x
        info.histoR.ywindowsize = event.y
        info.histoR.uwindowsize  = 1
    endif

    if(type eq 1) then begin
        info.histoZ.xwindowsize = event.x
        info.histoZ.ywindowsize = event.y
        info.histoZ.uwindowsize  = 1
    endif

    if(type eq 1) then begin
        info.histoS.xwindowsize = event.x
        info.histoS.ywindowsize = event.y
        info.histoS.uwindowsize  = 1
    endif
...
;***********************************************************************
;***********************************************************************
pro mql_display_histo,type,info
...
jintegration = 0 
if(type eq 0) then begin
    histo_uwindowsize = info.histoR.uwindowsize
    histo_xwindowsize = info.histoR.xwindowsize
    histo_ywindowsize = info.histoR.ywindowsize

    stitle = "MIRI Quick Look- Histo Science Frame Image" + info.version
    svalue = " Histogram of Science Frame Values "
    iframe = fix(info.histoR.iramp+1)
    jintegration = fix(info.histoR.jintegration+1)
    ...
    outname = '_'+ ij  + info.output.historaw 

    sxmin = strcompress(string(info.histoR.ximage_range[0]),/remove_all)
    sxmax = strcompress(string(info.histoR.ximage_range[1]),/remove_all)
    symin = strcompress(string(info.histoR.yimage_range[0]),/remove_all)
    symax = strcompress(string(info.histoR.yimage_range[1]),/remove_all)
    sregion = "Plot Region: range: " + sxmin + " - " + sxmax + " yrange: " + $
              symin + "  - " + symax + "  [No reference pixels included.]" 
endif

if(type eq 1) then begin
    histo_uwindowsize = info.histoZ.uwindowsize
    histo_xwindowsize = info.histoZ.xwindowsize
    histo_ywindowsize = info.histoZ.ywindowsize
    svalue = " Histogram of Zoomed Science Frame" 
    stitle = "MIRI Quick Look- Histo Zoomed Science Frame" + info.version
    if(info.image.zoom_window eq  3) then begin 
        svalue = " Histogram of Zoomed Reduced Image" 
        stitle = "MIRI Quick Look- Histo Zoomed Reduced Image" + info.version
    endif


    iframe = fix(info.histoZ.iramp+1)
    jintegration = fix(info.histoZ.jintegration+1)

    ftitle = "Integration #: " + strtrim(string(jintegration),2) +    "  Frame #: " + $
         strtrim(string(iframe),2)
    if( XRegistered ('mqlhz')) then begin
        widget_control,info.HistoZoomQuickLook,/destroy
    endif
    ...
    outname ='_'+ ij + info.output.histozoom  

    sxmin = strcompress(string(info.histoZ.ximage_range[0]),/remove_all)
    sxmax = strcompress(string(info.histoZ.ximage_range[1]),/remove_all)
    symin = strcompress(string(info.histoZ.yimage_range[0]),/remove_all)
    symax = strcompress(string(info.histoZ.yimage_range[1]),/remove_all)
    sregion = "Plot Region: range: " + sxmin + " - " + sxmax + " yrange: " + $
              symin + "  - " + symax + "  [No reference pixels included.]" 
endif
...
if(type eq 2) then begin
    histo_uwindowsize = info.histoS.uwindowsize
    histo_xwindowsize = info.histoS.xwindowsize
    histo_ywindowsize = info.histoS.ywindowsize
    stitle = "MIRI Quick Look- Histo Slope Image" + info.version
    svalue = " Histogram of Slope Values"
    jintegration = fix(info.histoS.jintegration+1)
    ftitle = " Integration #: " + strtrim(string(jintegration),2)
    if(XRegistered ('mqlhs')) then begin
        widget_control,info.HistoSlopeQuickLook,/destroy
    endif

    iframe = info.histoS.iramp+1


    ij = 'int' + string(jintegration)   
    ij = strcompress(ij,/remove_all)
    outname = '_'+ ij + info.output.histoslope 

    sxmin = strcompress(string(info.histoS.ximage_range[0]),/remove_all)
    sxmax = strcompress(string(info.histoS.ximage_range[1]),/remove_all)
    symin = strcompress(string(info.histoS.yimage_range[0]),/remove_all)
    symax = strcompress(string(info.histoS.yimage_range[1]),/remove_all)
    sregion = "Plot Region: range: " + sxmin + " - " + sxmax + " yrange: " + $
              symin + "  - " + symax + "  [No reference pixels included.]"  
endif
...
; add quit button
quitbutton = widget_button(quitmenu,value="Quit",event_pro='mql_histo_quit')
...
if(type eq 0) then $
XManager,'mqlhr',HistoQuickLook,/No_Block,event_handler='mql_histo_event'


if(type eq 1) then $
XManager,'mqlhz',HistoQuickLook,/No_Block,event_handler='mql_histo_event'

if(type eq 2) then $
XManager,'mqlhs',HistoQuickLook,/No_Block,event_handler='mql_histo_event'
...
if(type eq 0) then begin
    info.HistoRawQuickLook = HistoQuickLook
    Widget_Control,info.QuickLook,Set_UValue=info
    Widget_Control,info.HistoRawQuickLook,Set_UValue=hinfo
endif

if(type eq 1) then begin
    info.HistoZoomQuickLook = HistoQuickLook
    Widget_Control,info.QuickLook,Set_UValue=info
    Widget_Control,info.HistoZoomQuickLook,Set_UValue=hinfo
endif

if(type eq 2) then begin
    info.HistoSlopeQuickLook = HistoQuickLook
    Widget_Control,info.QuickLook,Set_UValue=info
    Widget_Control,info.HistoSlopeQuickLook,Set_UValue=hinfo
endif
