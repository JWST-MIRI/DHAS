...
pro msql_compare_histo_event,event
...
if (widget_info(event.id,/TLB_SIZE_EVENTS) eq 1 ) then begin
    minfo.scompare_histo.xwindowsize = event.x
    minfo.scompare_histo.ywindowsize = event.y
    minfo.scompare_histo.uwindowsize = 1
    widget_control,event.top,set_uvalue = hinfo
    widget_control,hinfo.info.Quicklook,set_uvalue = minfo
    msql_compare_histo,minfo
    return
endif
...
case 1 of
    (strmid(event_name,0,3) EQ 'Bin') : begin
        ...
        msql_update_compare_histo,graphno,hinfo
...
;        print,'plot number ', strmid(event_name,5,1)
;        print,'x y  ', strmid(event_name,6,1)
;        print,'min max ', strmid(event_name,7,1)
...
; channels scale individually
        ...
        if(hinfo.scaleimage-1 eq 3) then begin
            ...
            msql_update_compare_histo,k,hinfo
        endif
        ...
        if(hinfo.scaleimage-1 ne 3) then begin
            ...
            for i = 0,2 do begin
                ...
                widget_control,event.top,set_uvalue = hinfo
                widget_control,hinfo.info.Quicklook,set_uvalue = minfo
                msql_update_compare_histo,i,hinfo
            endfor
        endif
...
;_______________________________________________________________________
    ...
    (strmid(event_name,0,6) EQ 'ascale') : begin
        ...
        for i = 0,2 do begin
            ...
            if(hinfo.scaleimage-1 eq i) then begin
                widget_control,hinfo.histo_recomputeID[i,0],set_value='Plot Range '
                hinfo.default_scale_histo[i,0] = 1
                widget_control,hinfo.histo_recomputeID[i,1],set_value='Plot Range '
                hinfo.default_scale_histo[i,1] = 1
...
;***********************************************************************
pro msql_update_compare_histo,imageno,hinfo,ps=ps,eps=eps,ascii=ascii,unit=iunit
...
smedian = strcompress(string(info.rcompare_image[imageno].median),/remove_all) 
...
if(hinfo.default_scale_histo[index] eq 0) then begin
    ...
    xhistomin = info.rcompare_image[imageno].median - info.rcompare_image[imageno].stdev*3
    xhistomax = info.rcompare_image[imageno].median + info.rcompare_image[imageno].stdev*3
...
plot,xnew,h,psym=10,xtitle= xt,ytitle=yt,$
        ...
        ystyle=1,xstyle = 1,xticks = tick_no,title = stitle,ytickformat = '(f8.0)',xtickformat= '(f9.3)'
...
if(index ne 3) then begin
    if(index ne imageno) then begin
...
;***********************************************************************
pro msql_compare_histo,info

window,1,/pixmap
wdelete,1
if( XRegistered ('sCphisto')) then begin
    widget_control,info.HistoSCompareQuickLook,/destroy
endif
...
stitle = "MIRI Quick Look- Histogram of Comparison Slope Images" + info.version
...
if(info.scompare_histo.uwindowsize eq 1) then begin ; user has set window size 
    xsize_scroll = info.scompare_histo.xwindowsize
    ysize_scroll = info.scompare_histo.ywindowsize
endif
...
quitbutton = widget_button(quitmenu,value="Quit",event_pro='msql_compare_histo_quit')

scaleMenu = widget_button(menuBar,value="Lock Plot Ranges",font = info.font2)
scalebutton1 = widget_button(scalemenu,value=" Scale All Plots to Image 1",uvalue = 'ascale1')
scalebutton2 = widget_button(scalemenu,value=" Scale All Plots to Image 2",uvalue = 'ascale2')
scalebutton3 = widget_button(scalemenu,value=" Scale All Plots to Image 3",uvalue = 'ascale3')
...
graphID13 = widget_base(graphID_master1,col=1)
...
histo_xrange        = fltarr(3,2) ; x  plot range
histo_yrange        = lonarr(3,2) ; x  plot range

xrange_default        = fltarr(3,2) ; x  plot range
...
rawmedian = info.rcompare_image[0].median
...
rawmedian = info.rcompare_image[1].median
...
;*****
;titleID = widget_label(graphID13, value = " Histogram: Image 3: " + ftitle3,$
;                       /align_center,font=info.font3)
...
rawmedian = info.rcompare_image[2].median
...
XManager,'sCphisto',HistoQuickLook,/No_Block,event_handler='msql_compare_histo_event'
...
sint1 = strtrim( string (fix(info.rcompare_image[0].jintegration+1)),2)
sint2 = strtrim( string(fix(info.rcompare_image[1].jintegration+1)),2) 


ij = '_int' + sint1 + '_' + sint2
ij = strcompress(ij,/remove_all)

outname = 'histo_slope_compare' + ij + '_' 
...
info.HistoSCompareQuickLook = HistoQuickLook
Widget_Control,info.HistoSCompareQuickLook,Set_UValue=hinfo
Widget_Control,info.QuickLook,Set_UValue=info
for i = 0,2 do begin
    imageno = i
    msql_update_Compare_histo,imageno,hinfo
endfor
Widget_Control,info.HistoSCompareQuickLook,Set_UValue=hinfo
